...
gen = tcpip('192.168.1.101','RemotePort',5025)
fopen(gen)
...
for(k=1:Nf)
	...
	str1 = strcat('APPL:SIN ',{' '},num2str(f(k)),' HZ, 5.0 VPP, 0.0 V');
	fprintf(gen,char(str1));
	
	% --------- acq in -------
	[s,Ts]=readOsc1(osc,1/f(k));
	str2=strcat(dir_root,'/in/i',num2str(cont),'.mat');
	T(k) = Ts;
	save(str2,'s')

	% --------- acq out -------
	s=readOsc2(osc,1/f(k));
	str2=strcat(dir_root,'/out/i',num2str(cont),'.mat');
	save(str2,'s')
